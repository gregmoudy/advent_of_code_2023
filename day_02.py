...
class Solver(solver.Solver):
    def get_answer(self, data, part2 = False):
        ...
        valid_game_numbers = list()
        games_max_cubes_needed = dict()
        ...
        for line in data:
            ...
            for round in rounds:
                ...
                for cube_set in cubes:
                    ...
                    num = int(num)
                    color_max = COLOR_MAX_LOOKUP[color]
                    ...
                    # Part 2
                    game_max_cubes_needed = games_max_cubes_needed.setdefault(game_number, dict())
                    current_max = game_max_cubes_needed.get(color, 0)
                    game_max_cubes_needed[color] = max(num, current_max)
        ...
        # Part 1
        answer = sum(valid_game_numbers)

        # Part 2
        if part2:
            power_values = list()
            for game_max_cubes_needed in games_max_cubes_needed.values():
                values = list(game_max_cubes_needed.values())
                power = None
                for value in values:
                    if power is None:
                        power = value
