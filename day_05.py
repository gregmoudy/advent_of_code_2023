...
# but it starts at 50, so its two values are 50 and 51. With this information, you know that seed number 98 corresponds to soil number 50 and 
...
# Using these maps, find the lowest location number that corresponds to any of the initial seeds. To do this, 
...
# contains 14 values: 79, 80, ..., 91, 92. The second range starts with seed number 55 and contains 13 values: 55, 56, ..., 66, 67.
...
class Solver(solver.Solver):
    def get_answer(self, data, part2 = False):
        ...
        # Key tuple<source name, destination name>
        # Value list<[desitnation start, source start, range]>
        almanac = dict()
        seeds = list()

        current_section = None

        # Parse the data into basic data types we can quickly work with.
        for line in data:
            if not line:
                current_section = None

            elif 'map:' in line:
                # Tuple of (source name, destination name)
                current_section = tuple(line.split(' map:')[0].split('-to-'))
                ...
                #[desitnation start, source start, range]
                current_section_numbers = [int(x) for x in line.split(' ')]
                current_section_ranges = almanac.setdefault(current_section, list())
                current_section_ranges.append(current_section_numbers)


        def find_mapping_key_for_source_name(source_name):
            for key in almanac.keys():
                if key[0] == source_name:
                    return key


        def find_location_recursive(source_name, source_number):
            mapped_value = source_number

            mapping_key = find_mapping_key_for_source_name(source_name)
            mapping_ranges = almanac[mapping_key]
            for mapping_range in mapping_ranges:
                dest_start, src_start, dest_src_range = mapping_range
                if source_number in range(src_start, src_start + dest_src_range):
                    mapped_value = dest_start + (source_number - src_start)
                    break

            if source_name == 'humidity': # humidity-to-location is the last mapping.
                return mapped_value

            mapped_value = find_location_recursive(mapping_key[1], mapped_value)

            return mapped_value


        # Part 1
        # Go through each seed, work through the mappings, find lowest location number which is the last number in the mapping.
        if not part2:
            location_numbers = list()
            for seed in seeds:
                location_number = find_location_recursive('seed', seed)
