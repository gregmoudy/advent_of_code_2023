...
class Solver(solver.Solver):
    def get_answer(self, data, part2 = False):
        ...
        number_of_card_copies = dict()

        for line in data:
            card_number, numbers = line.split(':')
            winning_numbers, has_numbers = numbers.split('|')

            card_number = int(card_number.split(' ')[-1])
            winning_numbers = [int(x) for x in winning_numbers.split(' ') if x.isnumeric()]
            has_numbers = [int(x) for x in has_numbers.split(' ') if x.isnumeric()]

            has_winning_numbers = [x for x in has_numbers if x in winning_numbers]

            # Part 1
            if not part2:
                card_points = 0
                for _i in range(len(has_winning_numbers)):
                    if card_points == 0:
                        card_points = 1
                ...
                total_cards_points += card_points

            # Part 2
            else:
                card_copies_count = number_of_card_copies.setdefault(card_number, 1)
                for _i in range(card_copies_count):
                    for i in range(1, len(has_winning_numbers) + 1):
                        updated_card_copies_count = number_of_card_copies.setdefault(card_number + i, 1)
                        updated_card_copies_count += 1
                        number_of_card_copies[card_number + i] = updated_card_copies_count

        if not part2:
            answer = total_cards_points
        
        else:
            answer = sum(number_of_card_copies.values())
